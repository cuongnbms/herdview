...
- Never log, print or put in an error a token, a credential file's contents, or a response body. Logs carry Provider and outcome only.
...
- [ ] **Step 4: Hidden-window and not-signed-in checks by hand**

First, polling stops while hidden: with the app running, click the menu bar item until the window is hidden, wait 6 minutes, then run `log show --last 5m --predicate 'process == "herdview"' | grep quota`. Expected: no output. Show the window again and confirm new `quota` lines appear within a few seconds.
